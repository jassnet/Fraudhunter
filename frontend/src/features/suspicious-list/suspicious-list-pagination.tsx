...
export function SuspiciousListPagination({
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-x-3 gap-y-1 py-1 text-[12px] text-foreground/78">
      <div
        aria-label={suspiciousCopy.labels.resultRange}
        aria-live="polite"
        className="tabular-nums text-foreground/86"
      >
        {resultRange}
      </div>
      <div className="flex items-center gap-1">
        ...
        <span className="min-w-[3.5rem] text-center text-[12px] font-medium text-foreground/90 tabular-nums">
          {page} / {totalPages}
        </span>
        <Button
          type="button"
