...
export function OverviewChart({
  ...
  const maxByMetric = {
    ...
    suspicious_conversions: Math.max(
      ...rows.map((row) => safeNumber(row.suspicious_conversions)),
      1
    ),
  };
      ...
      <div className="min-w-[44rem] space-y-3">
        {metricConfig.map((metric, metricIndex) => {
          ...
          return (
            ...
            >
              <div className="flex min-w-0 flex-col justify-between border-r border-border/70 py-1 pr-3">
                <div className="space-y-1">
                  ...
                  <div className="text-[11px] text-muted-foreground">
                    {dashboardCopy.chart.maxLabel(metricMax)}
                  </div>
                </div>
              ...
              <div className="grid grid-cols-14 gap-2">
                {rows.map((row, index) => {
                  ...
                  const tooltipLabel = [
                    row.date,
                    `${dashboardCopy.chart.legends.clicks} ${safeNumber(row.clicks).toLocaleString("en-US")}`,
                    `${dashboardCopy.chart.legends.conversions} ${safeNumber(row.conversions).toLocaleString("en-US")}`,
                    `${dashboardCopy.chart.legends.suspiciousConversions} ${safeNumber(row.suspicious_conversions).toLocaleString("en-US")}`,
                  ].join("\n");
